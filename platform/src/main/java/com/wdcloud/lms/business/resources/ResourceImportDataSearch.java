...
@SelfDefinedFunction(
        resourceName = Constants.RESOURCE_TYPE_RES_IMPORT_SEARCH,
        functionName = Constants.FUNCTION_TYPE_PAGE_LIST
)
public class ResourceImportDataSearch implements ISelfDefinedSearch<PageInfoVO<ResourceVO>> {
    ...
    /**
     * @api {get} /resImportSearch/pageList/query 导入资源查询分页
     * @apiDescription 导入资源查询分页
     * @apiName resImportSearchPageList
     * @apiGroup resource
     *
    ...
    public PageInfoVO<ResourceVO> search(Map<String, String> condition) {
        ...
        if(ListUtils.isNotEmpty(types)){
            resourceVOS = (Page<ResourceVO>)resourceImportDao.getByUserIdAndTypeAndVersionMessageSort(userId, types);

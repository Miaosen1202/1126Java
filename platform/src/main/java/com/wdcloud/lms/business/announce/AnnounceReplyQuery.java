...
import com.wdcloud.lms.business.announce.dto.AnnounceReplyTree;
...
public class AnnounceReplyQuery implements IDataQueryComponent<AnnounceReply> {
    @Autowired
    private AnnounceReplyDao announceReplyDao;
    /**
     * @api {get} /announceReply/list 公告回复列表
     * @apiName announceReplyList
     * @apiGroup Announce
     *
     * @apiParam {Number} studyGroupId 学习小组ID
     * @apiParam {Number} announceId 公告ID
     * @apiParam {String} [content] 公告回复内容或作者名
     ...
     * @apiSuccess {Number} entity.announceId 关联公告ID
     ...
     */
    @Override
    public List<? extends AnnounceReply> list(Map<String, String> param) {
        //1.查回复列表
        //2.查用户信息 id头像昵称
        //3已读未读标识
        param.put("userId", WebContext.getUserId()+"");
        List<AnnounceReplyVO> list = announceReplyDao.findReply(param);
        //构造回复树结构
        return AnnounceReplyTree.getList(list);

    }
    /**
     * @api {get} /announceReply/pageList 公告回复分页列表
     * @apiName announceReplyPageList
     * @apiGroup Announce
     *
     * @apiParam {Number} studyGroupId 学习小组ID
     ...
     * @apiSuccess {Number} entity.list.announceId 关联公告ID
     ...
     */
    @Override
    public PageQueryResult<? extends AnnounceReply> pageList(Map<String, String> param, int pageIndex, int pageSize) {
        param.put("userId", WebContext.getUserId()+"");
        if (!param.containsKey("replyId")) {
            throw new ParamErrorException();
        }
        PageHelper.startPage(pageIndex,pageSize);
        Page<AnnounceReplyVO> list = (Page<AnnounceReplyVO>) announceReplyDao.findReply(param);

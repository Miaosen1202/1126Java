...
@SelfDefinedFunction(
        resourceName = Constants.RESOURCE_TYPE_RES_SHARE_SEARCH,
        functionName = Constants.FUNCTION_TYPE_PAGE_LIST
)
public class ResourceShareDataSearch implements ISelfDefinedSearch<PageInfoVO<ResourceVO>> {
    ...
    /**
     * @api {get} /resShareSearch/pageList/query 分享资源查询分页
     * @apiDescription 分享资源查询分页
     * @apiName resShareSearchPageList
     * @apiGroup resource
     *
     ...
     * @apiSuccess {Number} entity.list.favoriteCount 收藏次数
    ...
    public PageInfoVO<ResourceVO> search(Map<String, String> condition) {
        ...
        if(ListUtils.isNotEmpty(types)){
            resourceVOS = (Page<ResourceVO>)resourceDao.getByAuthorIdAndType(userId, types);
